    ...
    >
      <el-table-column label="主播ID" prop="id" align="center">
        <template slot-scope="{row}">
          <span>{{ row.anchor_id }}</span>
        </template>
      </el-table-column>
      <el-table-column label="主播姓名" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="认证标签" align="center">
        <template slot-scope="{row}">
          <span>{{ row.authentication | authFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="任务次数" align="center">
        <template slot-scope="{row}">
...
<script>
import { anchorlist, editstate } from '@/api/admin/account'
...
export default {
  filters: {
    ...
    authFilter(status) {
      ...
      return statusMap[status] ? statusMap[status] : '未认证'
    }
  },
  data() {
    return {
